...
#define EVENT_NAME L"\\BaseNamedObjects\\FileEvent"  //驱动层通知事件
#define APP_EVENT_NAME L"\\BaseNamedObjects\\FileAppEvent" //应用层是否事件
...
{
    HANDLE             FileHandle;        // 事件对象句柄
    PKEVENT            FileEvent;          // 用户和内核通信的事件对象指针
	CHAR			ProcName[256];					//进程名
    CHAR			ProcFullPath[256];         //进程路径
	CHAR   		FilePre[256];					//信息1
	CHAR			FilePath[256];					//信息2
